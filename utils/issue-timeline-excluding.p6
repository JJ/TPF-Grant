...
for glob("../data/issues/*.json") -> $file {
    ...
    my $labels = ($data<labels>.map: *<name>).Set;
    say "Intersection ", set("external","wishlist","NOTSPECCED") ∩ $labels;
    next if set("external","wishlist","NOTSPECCED") ∩ $labels;
    say "Goes through ", $labels;
    CATCH {
	default {
	    say $file.IO.slurp;
	    say .backtrace;
...
spurt("../data/issues-timeline-excluded.csv", @open-issues.join("\n"));
...
spurt( "../data/issues-month-excluded.csv", @issues-month.join("\n") );
